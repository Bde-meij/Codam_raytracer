...
t_render_params	*sample_config(void)
{
	...
	render_params->light = point_light_array_new(1);
	point_light_array_append(&render_params->light, point_light_new(vec3_new(-1, 0, 1), vec3_new(1, 1, 1), 0.8));
	point_light_array_append(&render_params->light, point_light_new(vec3_new(0, 1, 1), vec3_new(1, 1, 1), 0.8));
